...
bool solveSudoku(vector<vector<int>>& board) {
    for (int row = 0; row < 9; row++) { // cari sel kosong dengan nested for
        for (int col = 0; col < 9; col++) { 
            if (board[row][col] == 0) { // Sel kosong ditemukan
                for (int num = 1; num <= 9; num++) { // Coba masukkan angka 1-9
                    if (isValid(board, row, col, num)) { // Periksa validitas
                        board[row][col] = num; // Masukkan angka

                        if (solveSudoku(board)) { // Rekursi
                            return true; // Rekursi berhasil
                        }

                        board[row][col] = 0; // Jika gagal, kosongkan sel
                    }
                }
                return false; // Tidak ada angka yang valid, backtrack
            }
        }
    }
    return true; // Solusi telah ditemukan (tidak ada sel kosong)
}
